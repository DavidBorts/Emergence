...
OscMsg msg;
7401 => oin.port;
oin.addAddress("/note, f, f, f");

fun void receiveUDP() {
    while(true) {
        oin => now;
        while(oin.recv(msg)) {
            Std.ftoi(msg.getFloat(0)) => int f;
            msg.getFloat(1) => float d;
            Std.ftoi(msg.getFloat(2)) => int n;
            0.0 => float b;
            f => int test;
            ...
            if(n == 0) {
                Piano p;
                spork~ p.playNote(f, d);
            } else if(n == 1) {
                Bells b;
                spork~ b.playNote(f, d);
                <<<"Bells">>>;
            } else if(n == 2) {
                Strings s;
                spork~ s.playNote(f, d, 1);
            } else if(n == 3) {
                Harps h;
                spork~ h.playNote(f, d);
            } else if(n == 4) {
                Flutes f;
                spork~ f.playNote(test, d, 1);
            } else {
                Clarinets c;
                spork~ c.playNote(f, d, 2.0);
            } 
        }
    }
}
